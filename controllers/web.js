...
router.get('/dashboard', checkLogin, async (req, res) => {
    ...
    res.render("dashboard", { laptops: laptops, username: username, role: role, error: error, success: success });
});
// Detail
router.get('/laptops/:id', checkLogin, async (req, res) => {
    const id = req.params.id;
    const laptop = await Laptop.findOne({ _id: id });
    res.render("detail", { laptop: laptop });
});
router.post('/laptops/delete/:id', checkLogin, checkRole, async (req, res) => {
    const id = req.params.id;
    try {
...
router.post('/laptops', checkLogin, checkRole, async (req, res) => {
...
router.post('/laptops/update/:id', checkLogin, checkRole, async (req, res) => {
    const { title, price, description } = req.body;
    const id = req.params.id;
    try {
        await Laptop.updateOne({ _id: id }, { title, price, description });
        req.session.success = true
    } catch (err) {
        req.session.error = true
    }
